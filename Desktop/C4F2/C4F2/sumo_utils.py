...
def run_episode(conn, agent, competition_round, train=True):
    ...
    while conn.simulation.getMinExpectedNumber() > 0 and step <= MAX_STEP_COUNT:
        ...
        if agent is not None:
            ...
            prev_reward = get_waiting_count(conn,vehicle_ids)
            prev_time=state[1]+state[2]+state[3]+state[4]+state[5]+state[6]+state[7]+state[8]
            cur_waiting_time, elapsed, emissions = take_action(conn, state, action, competition_round)
            state=get_state(conn, competition_round)
            curr_time=state[1]+state[2]+state[3]+state[4]+state[5]+state[6]+state[7]+state[8]
            curr_reward = get_waiting_count(conn,vehicle_ids)
            ...
            elapsed = 1
            conn.simulationStep()
        prev_total_waiting_time=total_waiting_time
        total_waiting_time += cur_waiting_time

        reward=get_moving_count(conn,vehicle_ids)-get_waiting_count(conn,vehicle_ids)
